...
Now we know that we have a daemon with the name EPMD that we familiarized with their responsibility and, furthermore, we know that EDP is a protocol that defines some messages and has some specifications (like it listens on TCP port 4369), this Daemon and Protocol help Erlang nodes to find each other in the cluster. If we want to get some information about clustered nodes from EPMD this shell command can help us (please care 127.0.0.1):  
...
`workers`: How many workers must exist on each channel. `worker` gets a request from one `conn` and processes it and prepares the result, then sends the result as a reply to `conn` for sending to the requested node.

Here arise two main questions: 
#### 1. Why do we have workers and why we must not process the incoming request with actors that hold a socket?
If we process each incoming request with `conn`, during the process of the received request the `conn` actor is blocked and other received requests are queued in `conn`'s inbox, and this approach increases the response time for each request. 
Bingo uses a scalable approach and tries to decrease response time. When one `conn` receives a request, first it selects a worker randomly and sends that request to that worker. After a selected worker processes one request, it sends the result to `conn` (because `conn` holds the socket). `conn` transfers the result to the requested node as a reply.

#### 2. How Bingo does `multi_call` and `multi_cast` requests?
To respond to this question, first, we must be familiar with `multi_cast` and `multi_call` anatomy! With `multi_x` API Bingo, sends specific requests to a list of nodes in the cluster and gathers results, and then returns replies. 
Bingo hires a set of workers and named them: `mc.workers`. When Bingo receives `multi_call` or `multi_cast` request, it selects one `mc_worker` and sends that request to a selected worker. This worker sends `call` or `cast` requests to all requested remote nodes concurrently waits to gather results prepares a reply and returns it.
Bingo provides the below options to configure `multicast` and `multi call` behaviors:
...
`mc.workers`: How many `multi_x` workers must exist on one channel?

`mc.timeout`: How much time one worker must be waiting for gathering results from all nodes.

Configuration Options
=====================
The below snippet shows all configuration options and if we want to use Bingo in our application these options must be set carefully based on application requirements.
``` erlang
...
`ping/1`: Gets the remote node name as input sends `ping` to that node and returns the result.</br>
`multi_ping/1`: Gets a list of remote nodes' names sends `ping` to those nodes and returns results.</br>
...
`cast/2`: Applies `erlang:F/0` on the target node and returns `ok` if the target node is available.</br>
`cast/3`: Applies `erlang:F/0` with timeout or `M:F/0` on the target node and returns `ok` if the target node is available.</br>
`cast/4`: Applies `M:F/0` with timeout or `M:F/A` on the target node and returns `ok` if the target node is available.</br>
`cast/5`: Applies `M:F/A` with timeout on the target node and returns `ok` if the target node is available.</br>
...
Each of the above APIs has complete comments and specs for other details. If you require more documentation, please visit the [bingo.erl](https://github.com/mohsenmoqadam/bingo/blob/main/src/bingo.erl) file.
